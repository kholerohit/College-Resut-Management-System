...
<?php
	if(isset($_POST["submit"]))
	{
		...
		{
			$_SESSION["ErrorMessage"]="password should between 3 to 20 chars";
			header("Location:loginteacher.php");
			exit;
		}
		elseif(strlen($password)<8 || strlen($password)>20 )
		{
			$_SESSION["ErrorMessage"]="password should between 8 to 20 chars";
			header("Location:loginteacher.php");
			exit;
		}
		...
		{
			global $conn;
			$query = "INSERT INTO login_teacher(datetime,username,password,addedby)
	...
	<div class="row">
		...
		<h1 class="sidebar-brand">The<br><?php echo ucwords($_SESSION["Username"]) ;?></h1>
						      ...
						      <li class="nav-item mt-auto btn ">
						        <a class="nav-link  btn btn-outline-success" href="dashboard.php">
						          <i class=""></i> Dashboard
									...
									<li class="nav-item mt-auto btn ">
										<a class="nav-link btn btn-outline-success " href="teacher_board.php">
                  ...
                  <li class="nav-item btn ">
						      ...
						      <li class="nav-item mt-auto btn">
						        <a class="nav-link btn btn-outline-success" href="Blog.php">
						          <i class="nav-icon cui-cloud-download"></i>Live Blog</a>
						      </li>
						      <li class="nav-item btn">
						        <a class="nav-link btn btn-primary" href="loginteacher.php">
			...
			<div>
				<form action="loginteacher.php" method="POST">
					...
					<div class="form-group">
						...
						<input class="form-control col-sm-4" type="text" name="username" id="username" placeholder="username">
					</div>
					<div class="form-group">
						<label class="fieldinform" for="password">Password</label>
						<input class="form-control col-sm-4" type="password" name="password" id="password" placeholder="password">
					</div>
					<div class="form-group">
						<label class="fieldinform"  for="re_password">Confirm Password</label>
						<input class="form-control col-sm-4" type="password" name="re_password" id="re_password" placeholder="Confirm password">
					</div>
			...
			<div class="table-responsive ">
				<table class="table table-striped table-hover">
					<tr>
						...
						<th class="tht">Teacher Name </th>
						<th class="tht">Added BY</th>
						<th class="tht"> Action</th>
					</tr>
				<?php
				global $conn;
				$query = "SELECT * FROM login_teacher ORDER BY id DESC";
				$execute = mysqli_query($conn,$query) or die('Error');
				$sr_no = 0;
				...
				<tr>
					...
					<td><?php echo htmlentities(ucwords($username)); ?></td>
					<td><?php echo htmlentities($admin); ?></td>
					<td><a href="teacher_delete.php?id=<?php echo $id; ?>"><span class="btn btn-danger">Delete</span></a></td>
				</tr>
				<?php } ?>
				</table>
			</div>
